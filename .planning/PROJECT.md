...
v1.3에서 달성한 것:
- SessionStore 복합 키(`instanceID + callID`) 기반 멀티 다이얼로그 지원
- MuteTransfer Command 노드 — Replaces REFER + final NOTIFY 대기 + 양쪽 BYE 정리
- Transfer 수신 측 Replaces REFER 처리 후 새 dialog로 교체
- Command/Event Properties 패널 공통 `callId` 필드 추가
