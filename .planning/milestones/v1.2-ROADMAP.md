# SIPFLOW v1.2 Roadmap Archive — Transfer + UI 개선

**Released:** 2026-02-20
**Duration:** 2026-02-19 ~ 2026-02-20 (2일)
**Stats:** 4 phases, 7 plans, 32 commits, 50 files changed (+10,380 / -2,738)
...
1. **Hold/Retrieve 통화 보류/해제** — AnswerOptions 리팩토링 + SIP 이벤트 버스 인프라 + Re-INVITE 기반 Hold/Retrieve Command/Event
2. **BlindTransfer 블라인드 전환** — diago Refer() API + OnRefer 콜백 완전 구현 (Refer-To URI 추출, Invite/Ack, SessionStore 교체)
3. **UI 리디자인 (Activity Bar + Resizable)** — 고정 사이드바 → Activity Bar 토글 + shadcn Resizable 너비 조절 패널
4. **6개 새 노드 프론트엔드 UI 완성** — Hold, Retrieve, BlindTransfer, HeldEvent, RetrievedEvent, TransferEvent 아이콘/팔레트/Properties
5. **Properties 패널 자동 토글** — 노드 선택 시 expand, 해제 시 collapse
6. **56+ Go 테스트 + v1.1 하위 호환성 검증** — 파싱 테스트, 에러 경로 테스트, v1.1 시나리오 호환성
...
| Phase | 결정 | 이유 |
|-------|------|------|
| 10 | OnMediaUpdate goroutine 분리 | diago d.mu.Lock() 재진입 데드락 방지 |
| 10 | non-blocking SIP 이벤트 버스 | 버퍼 1 채널 + default 케이스, 최신 이벤트 보장 |
| 10 | reInviter 인터페이스 로컬 정의 | diago DialogSession에 ReInvite() 미포함 |
| 10 | Hold 실패 시 Mode 복원 | sendrecv로 복원하여 미디어 세션 일관성 유지 |
| 11 | referrer 인터페이스 로컬 정의 | Phase 10 reInviter 패턴과 동일 방식 적용 |
| 11 | REFER 후 즉시 BYE 전송 | BlindTransfer 완료 후 호출자 이탈 필요 |
| 11 | Recipient 필드로 URI 추출 | sip.Request에 RequestURI() 메서드 없음 |
| 11 | StoreDialog 후 emitSIPEvent | 후속 노드가 올바른 dialog 사용 보장 |
| 12 | panelRef prop 사용 | react-resizable-panels v4, React 18 호환 |
| 12 | onResize collapse 감지 | v4에 onCollapse 미존재, asPercentage === 0 |
| 13 | Properties 패널 auto-expand/collapse | 노드 미선택 시 캔버스 공간 최대화 |
| 13 | BlindTransfer 인라인 표시 To: user@host | MakeCall targetUri 패턴과 동일 UX |

## 기술 부채 (다음 마일스톤으로 이월)

| 항목 | 심각도 | 설명 |
|------|--------|------|
| AttendedTransfer 미구현 | 중간 | SessionStore 복합 키 리팩토링 + diago Replaces 미지원 |
| NOTIFY Event 노드 미구현 | 낮음 | Transfer 진행 상태 추적 (AttendedTransfer와 함께) |
| Frontend validation.ts | 낮음 | BlindTransfer targetUser/targetHost 사전 검증 누락 |
| Executor 성공 경로 커버리지 | 낮음 | 실 SIP 서버 필요로 낮은 커버리지 |
