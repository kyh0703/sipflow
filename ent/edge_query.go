...
type EdgeQuery struct {
	...
	withFlow       *FlowQuery
	withSourceNode *NodeQuery
	withTargetNode *NodeQuery
	withFKs        bool
...
func (_q *EdgeQuery) QueryFlow() *FlowQuery {
...
func (_q *EdgeQuery) QuerySourceNode() *NodeQuery {
...
func (_q *EdgeQuery) QueryTargetNode() *NodeQuery {
	query := (&NodeClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *EdgeQuery) WithFlow(opts ...func(*FlowQuery)) *EdgeQuery {
	query := (&FlowClient{config: _q.config}).Query()
...
func (_q *EdgeQuery) WithSourceNode(opts ...func(*NodeQuery)) *EdgeQuery {
	query := (&NodeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *EdgeQuery) WithTargetNode(opts ...func(*NodeQuery)) *EdgeQuery {
	query := (&NodeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *EdgeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Edge, error) {
	...
	if _q.withFlow != nil || _q.withSourceNode != nil || _q.withTargetNode != nil {
		withFKs = true
	}
	if withFKs {
...
func (_q *EdgeQuery) loadFlow(ctx context.Context, query *FlowQuery, nodes []*Edge, init func(*Edge), assign func(*Edge, *Flow)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Edge)
	for i := range nodes {
		if nodes[i].flow_edges == nil {
			continue
		}
		fk := *nodes[i].flow_edges
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "flow_edges" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *EdgeQuery) loadSourceNode(ctx context.Context, query *NodeQuery, nodes []*Edge, init func(*Edge), assign func(*Edge, *Node)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Edge)
	for i := range nodes {
		if nodes[i].source_node_id == nil {
			continue
		}
		fk := *nodes[i].source_node_id
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
...
func (_q *EdgeQuery) loadTargetNode(ctx context.Context, query *NodeQuery, nodes []*Edge, init func(*Edge), assign func(*Edge, *Node)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Edge)
	for i := range nodes {
		if nodes[i].target_node_id == nil {
			continue
		}
		fk := *nodes[i].target_node_id
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
