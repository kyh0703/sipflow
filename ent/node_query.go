...
type NodeQuery struct {
	...
	withFlow          *FlowQuery
	withOutgoingEdges *EdgeQuery
	withIncomingEdges *EdgeQuery
	withFKs           bool
...
func (_q *NodeQuery) QueryFlow() *FlowQuery {
	query := (&FlowClient{config: _q.config}).Query()
...
func (_q *NodeQuery) QueryOutgoingEdges() *EdgeQuery {
...
func (_q *NodeQuery) QueryIncomingEdges() *EdgeQuery {
	query := (&EdgeClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
// WithFlow tells the query-builder to eager-load the nodes that are connected to
// the "flow" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *NodeQuery) WithFlow(opts ...func(*FlowQuery)) *NodeQuery {
	query := (&FlowClient{config: _q.config}).Query()
...
func (_q *NodeQuery) WithOutgoingEdges(opts ...func(*EdgeQuery)) *NodeQuery {
	query := (&EdgeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *NodeQuery) WithIncomingEdges(opts ...func(*EdgeQuery)) *NodeQuery {
	query := (&EdgeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *NodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Node, error) {
	...
	if query := _q.withFlow; query != nil {
		if err := _q.loadFlow(ctx, query, nodes, nil,
			func(n *Node, e *Flow) { n.Edges.Flow = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withOutgoingEdges; query != nil {
		if err := _q.loadOutgoingEdges(ctx, query, nodes,
			func(n *Node) { n.Edges.OutgoingEdges = []*Edge{} },
			func(n *Node, e *Edge) { n.Edges.OutgoingEdges = append(n.Edges.OutgoingEdges, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withIncomingEdges; query != nil {
		if err := _q.loadIncomingEdges(ctx, query, nodes,
			func(n *Node) { n.Edges.IncomingEdges = []*Edge{} },
			func(n *Node, e *Edge) { n.Edges.IncomingEdges = append(n.Edges.IncomingEdges, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *NodeQuery) loadFlow(ctx context.Context, query *FlowQuery, nodes []*Node, init func(*Node), assign func(*Node, *Flow)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Node)
	for i := range nodes {
		if nodes[i].flow_nodes == nil {
			continue
		}
		fk := *nodes[i].flow_nodes
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "flow_nodes" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *NodeQuery) loadOutgoingEdges(ctx context.Context, query *EdgeQuery, nodes []*Node, init func(*Node), assign func(*Node, *Edge)) error {
	...
	query.Where(predicate.Edge(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(node.OutgoingEdgesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.source_node_id
		if fk == nil {
			return fmt.Errorf(`foreign-key "source_node_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (_q *NodeQuery) loadIncomingEdges(ctx context.Context, query *EdgeQuery, nodes []*Node, init func(*Node), assign func(*Node, *Edge)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Node)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Edge(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(node.IncomingEdgesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.target_node_id
		if fk == nil {
			return fmt.Errorf(`foreign-key "target_node_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
