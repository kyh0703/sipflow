...
// The SIPServerFunc type is an adapter to allow the use of ordinary
// function as SIPServer mutator.
type SIPServerFunc func(context.Context, *ent.SIPServerMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f SIPServerFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.SIPServerMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.SIPServerMutation", m)
}
