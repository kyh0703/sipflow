...
import { Save, Check, Circle, Loader2, Undo2, Redo2 } from 'lucide-react';
...
import { ScenarioStoreProvider, useScenarioStore } from '../store/scenario-store';
import { ExecutionStoreProvider, useExecutionStore } from '../store/execution-store';
...
function ScenarioBuilderLayout() {
        ...
        <div className="h-10 border-b border-border bg-background flex items-center justify-between px-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={undo}
              disabled={!canUndo}
              className="inline-flex h-7 w-7 items-center justify-center rounded border border-border hover:bg-accent disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              title="Undo (Ctrl+Z)"
            >
              <Undo2 size={14} />
            </button>
