...
must_haves:
  truths:
    - "2자 통화 시나리오(발신→응답→종료)가 시뮬레이션 모드에서 완료까지 정상 실행된다"
    - "시나리오 실행 중 노드 상태 변경 및 액션 로그 이벤트가 정상 발행된다"
    - "시나리오 완료 후 cleanup이 수행되어 재시작이 가능하다"
    - "go test ./internal/... 전체 통과로 엔진 안정성이 검증된다"
  artifacts:
...
@docs/exec-plans/completed/legacy-phases/05-ui-completion/CONTEXT.md
@docs/exec-plans/completed/legacy-phases/05-ui-completion/05-RESEARCH.md
...
<output>
완료 후, `docs/exec-plans/completed/legacy-phases/05-ui-completion/05-03-SUMMARY.md` 생성
</output>
